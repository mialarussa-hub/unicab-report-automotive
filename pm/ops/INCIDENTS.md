...
- **Follow-up:**
  ...
  - **Esposizione secondaria — RISOLTA (2026-07-02):** `api` era su `0.0.0.0:8000` e c'era un mapping stray `0.0.0.0:5678` sul container nginx (drift). Entrambi raggiungibili pubblicamente ma già serviti via nginx (443 → proxy interno). Fix: `api` e `n8n` legati a `127.0.0.1` (commit `981f7e4` nel repo + patch in-place su `/opt/unicab`, backup `docker-compose.yml.bak-ports`), `docker compose up -d --force-recreate api nginx` (compose ha riallineato anche `n8n`). Verificato: le uniche porte pubbliche rimaste sono `80`/`443`; `8000`/`5678`/`5432` solo loopback; sito HTTP 307 OK.

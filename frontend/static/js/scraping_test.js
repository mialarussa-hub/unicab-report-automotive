...
// L2YT e' una variante di L2 ristretta a youtube_editorial: lato render
// (filtro livello, banner motore) viene trattata come L2; il badge L2YT
// resta visibile nelle session card.
...
function renderResults(data, fromSession = false) {
    ...
    for (const level of LEVELS) {
        ...
        if (level.key === 'L2' && data.l2_synthesis && typeof data.l2_synthesis === 'object') {
            const synthCard = renderL2SynthesisCard(data.l2_synthesis);
        ...
        if (level.key === 'L3' && data.l3_synthesis && typeof data.l3_synthesis === 'object') {
            const synthCard = renderL3SynthesisCard(data.l3_synthesis);
            if (synthCard) body.appendChild(synthCard);
        }
...
function renderL2SynthesisCard(synth) {
...
function renderL3SynthesisCard(synth) {
...
// Piacere di guida". Dedup per (versione, alimentazione, cilindrata_cc).
...
function renderOfficialInfo(info, items = []) {
    ...
    if (info.is_driver_analysis) {
        return renderDriverAnalysis(info, items);
    }
